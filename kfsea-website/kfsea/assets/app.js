...
function buildFooter(){
  const col = (h,items) => `<div><h4>${h}</h4><ul class="foot-list">${items.map(([t,u])=>`<li><a href="${u}">${t}</a></li>`).join('')}</ul></div>`;
  return `
    ...
    <div class="wrap">
      <div class="foot-grid">
        <div class="footbrand">
          ...
          <p>Integrated fire protection, safety systems and training for schools, workplaces and institutions across East Africa.</p>
          <div class="chips" style="margin-top:16px">
            <span class="chip">BS 5839</span><span class="chip">BAFE SP203</span><span class="chip">KEBS</span><span class="chip">OSHA / NEMA</span>
          </div>
        </div>
        ${col('Solutions',[['Fire Safety','#/fire-safety'],['Security Systems','#/security-systems'],['Service &amp; Maintenance','#/service-maintenance'],['Equipment Store','#/products'],['Industries','#/industries']])}
        ${col('Training',[['Training Programmes','#/training'],['First Aid','#/first-aid'],['Fire Drills','#/fire-drills'],['Fire Marshals Club','#/fire-marshals-club'],['School Package','#/schools']])}
        <div>
          <h4>Contact</h4>
          <ul class="foot-list">
            <li><a href="tel:${CO.tel1Raw}">${CO.tel1}</a></li>
            <li><a href="tel:${CO.tel2Raw}">${CO.tel2}</a></li>
            <li><a href="mailto:${CO.email}">${CO.email}</a></li>
            <li><a href="https://wa.me/${CO.wa}" target="_blank" rel="noopener">WhatsApp</a></li>
            <li><a href="https://${CO.site}" target="_blank" rel="noopener">${CO.site}</a></li>
            <li>${CO.office}</li>
            <li>${CO.box}<br>${CO.city}</li>
            <li>${CO.hours}</li>
          </ul>
        </div>
      </div>
      ...
      <div class="foot-bottom">
        ...
        <p>${CO.site} &middot; Nairobi, Kenya</p>
      </div>
    </div>
  </footer>
...
function initHero(){
  ...
  video.src = small ? 'assets/video/kadala-hero-sm.mp4' : 'assets/video/kadala-hero.mp4';
  video.load();
  ...
  const p = video.play();
  if(p && p.catch) p.catch(()=>{ if(toggle) toggle.setAttribute('aria-pressed','true'); });
...
function closeAllMenus(){
  ...
  if(d){ d.classList.remove('open'); const b = document.getElementById('burger'); if(b) b.setAttribute('aria-expanded','false'); }
}
...
function wireChrome(){
  ...
  document.addEventListener('click', e=>{ if(!e.target.closest('.nav-item')) closeAllMenus(); });
