...
/* One line, one action. Placed after the equipment grid and again after the
   training block, which is where a reader is closest to enquiring. */
const qstrip = (h,p,label,href) => `
  <section class="qstrip">
    <div class="wrap">
      <div class="qstrip-t reveal">
        <h3>${h}</h3>
        <p>${p}</p>
      </div>
      <div class="btn-row reveal" data-d="1">
        ${btn(label,href)}
        <a class="qstrip-num" href="tel:${CO.tel1Raw}">
...
window.KFSEA_H2 = {story,band,marquee,matrix,wideCta,qstrip,P,PARTNERS};
...
PAGES['home'] = {
  ...
  ${qstrip('Tell us what is in the building. We will specify it.',
    'Send us the rooms, the risks and the floor count. You get a written specification and a price, not a catalogue.',
    'Get a quote','#/quote')}
    ...
    <div class="wrap">
      ...
      <div class="trainer reveal" id="trainer">
        <div class="trainer-media">
          ${[{img:P.training},{img:IMG.wallSign},{img:IMG.firstaidKit},{img:IMG.assemblySign},{img:IMG.hoseInterior}]
  ...
  <section class="section section--sm section--mist">
    <div class="wrap">
      ${secHead('Trust and credibility','The standards we work to','Every system we supply is specified against a named standard, and we publish only the standards our documentation references.',true)}
      <div class="grid g4">
...
PAGES['fire-protection'] = {
  ...
  <section class="section">
    <div class="wrap">
      ...
      <div class="grid g4 tilt-wrap">
        ${product({img:IMG.extPowder,h:'ABC Dry Powder',p:'General purpose cover for mixed risks, rated for Class A, B and C.',chips:['KEBS certified'],app:'Offices, schools, corridors, vehicles',order:'ABC Dry Powder Extinguisher',over:'Class A, B and C'})}
        ...
        ${product({img:IMG.extWater,h:'Water and Water Mist',p:'Cools burning solids. Mist variants reduce the risk around electrical equipment.',chips:['KEBS certified'],app:'Stores, dormitories, paper and textile risks',order:'Water Mist Extinguisher',over:'Class A'})}
      </div>
    </div>
  </section>
  ...
  <section class="section section--mist">
    <div class="wrap">
      ...
      <div class="split" style="margin-top:clamp(34px,4vw,56px)">
        <div class="figure figure--wide reveal">
          <img src="${IMG.panelSuppression}" alt="A fire inside an electrical distribution panel fitted with an automatic suppression cylinder" loading="lazy" decoding="async">
          <div class="figure-cap">Electrical risk is handled by the medium, not by a class letter</div>
        </div>
        <div class="reveal" data-d="1">
          ${eyebrow('Electrical risk')}
          <h3 style="margin-bottom:14px">A distribution board is a fuel store with a supply attached</h3>
          <p class="dim">Electrical is not a fire class. What burns is the insulation, the dust and the cable, and the supply keeps feeding it until somebody isolates it. That is why the medium has to be non conductive and why the isolation point has to be known before the fire, not found during it.</p>
  ...
  <section class="section">
    <div class="wrap">
      ...
      <div class="grid g3 tilt-wrap">
        ${product({img:IMG.blanket,h:'Fire Blankets',p:'Smothers a small fire at its source, and can be used on a person whose clothing has caught alight.',chips:['1m x 1m to 1.8m x 1.8m'],app:'Kitchens, laboratories, restaurants, schools',order:'Fire Blankets',over:'Wall mounted pouch'})}
  ...
  <section class="section">
    <div class="wrap">
      ...
      <div class="grid g3 tilt-wrap">
        ...
        ${product({img:IMG.detectorHead,fill:true,h:'Heat Detectors',p:'For kitchens, plant rooms and dusty stores where smoke detection would false alarm.',app:'Kitchens, plant rooms, workshops',order:'Heat Detectors',over:'Avoids nuisance alarms'})}
  ...
  <section class="section">
    <div class="wrap">
      ...
      <div class="split">
        ...
        <div class="art-frame art-frame--photo reveal" data-d="1"><img src="${IMG.alarmWall}" alt="A fire alarm control panel being operated on a commissioned system" loading="lazy" decoding="async"></div>
      </div>
...
PAGES['emergency-systems'] = {
  ...
  <section class="section">
    <div class="wrap">
      <div class="grid g3 tilt-wrap">
        ${product({img:IMG.emgLight,fill:true,h:'Emergency Lighting',p:'Maintained and non maintained luminaires that illuminate escape routes when the mains supply drops.',chips:['OSHA / NEMA','LED'],spec:['Exit lights and luminaires','Directional signage','Testing and maintenance'],app:'Stairwells, corridors, exits',order:'Emergency Lighting',over:'Tested and maintained'})}
        ${product({img:IMG.exitCorridor,fill:true,h:'Exit and Directional Signs',p:'Photoluminescent and LED signage that stays readable when the lights and the air are not on your side.',chips:['Photoluminescent'],spec:['Final exit signage','Directional route signs'],app:'All building types',order:'Exit Signage',over:'Readable without power'})}
        ${product({img:IMG.firepoint,fill:true,h:'Fire Point Signs',p:'Marks extinguisher and fire point locations so they can be found by someone who has never looked before.',chips:['Photoluminescent'],app:'Corridors, workshops, kitchens',order:'Fire Point Signage',over:'Makes equipment findable'})}
        ${product({img:IMG.assemblySign,h:'Assembly Point Signs',p:'Defines where people gather after evacuation, which is what turns a drill into a roll call.',app:'Car parks, fields, courtyards',order:'Assembly Point Signage',over:'Supports the roll call'})}
        ...
        ${product({img:IMG.sounder,h:'Sounders and Strobes',p:'Audible and visual warning devices, with standby batteries so the warning survives a mains failure.',chips:['BS 5839'],app:'All system types',order:'Sounders and Strobes',over:'Backup batteries'})}
      </div>
    </div>
  </section>
