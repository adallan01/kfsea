...
PAGES['fire-safety'] = {
  ...
  <section class="section">
    <div class="wrap">
      ...
      <div class="grid g4 tilt-wrap">
        ${product({art:G.svgExtinguisher('powder'),h:'ABC Dry Powder',p:'General purpose cover for mixed risks. The default choice for corridors, classrooms and vehicles.',chips:['KEBS certified','Refillable'],app:'Offices, schools, corridors, vehicles',order:'ABC Dry Powder Extinguisher',over:'Class A, B and C risks'})}
        ...
        ${product({art:G.svgExtinguisher('water'),h:'Water and Water Mist',p:'Cools burning solids. Water mist variants reduce the risk around electrical equipment.',chips:['KEBS certified'],app:'Stores, dormitories, paper and textile risks',order:'Water Mist Extinguisher',over:'Class A risks'})}
      </div>

      <div class="grid g2 tilt-wrap" style="margin-top:22px">
        ${product({img:IMG.rangeGroup,fill:true,h:'Mobile Trolley Units',p:'High capacity wheeled extinguishers for large open risks where a portable unit does not give enough coverage.',chips:['KEBS certified','Wheeled'],app:'Factories, yards, fuel bays, plant rooms',order:'Mobile Trolley Extinguisher',over:'For large open risk areas'})}
        ${product({art:G.svgBlanket(),h:'Fire Blankets',p:'Smothers a small fire at its source, and can be used on a person whose clothing has caught alight.',chips:['1m x 1m to 1.8m x 1.8m','Wall mounted'],app:'Kitchens, laboratories, restaurants, schools, offices, homes',order:'Fire Blankets',over:'Fire resistant, wall mounted pouch'})}
      </div>
    </div>
  </section>
  ...
  <section class="section">
    <div class="wrap">
      ...
      <div class="grid g3 tilt-wrap">
        ...
        ${product({art:G.svgSounder(),h:'Sounders and Accessories',p:'Panels, detectors, call points, sirens and backup batteries: the parts that make an alarm into a system.',chips:['BS 5839'],spec:['Sounders and strobes','Backup batteries','Control panel accessories'],app:'All system types',order:'Fire Alarm Accessories',over:'Backup batteries included'})}
      </div>
  ...
  <section class="section">
    <div class="wrap">
      ...
      <div class="grid g3 tilt-wrap">
        ${product({art:G.svgEmergencyLight(),h:'Emergency Lighting',p:'Maintained and non maintained luminaires that illuminate escape routes when the mains supply drops.',chips:['OSHA / NEMA','LED'],spec:['Emergency exit lights and luminaires','Directional signage','Testing and maintenance'],app:'Stairwells, corridors, exits',order:'Emergency Lighting',over:'Tested and maintained'})}
...
PAGES['security-systems'] = {
  ...
  <section class="section">
    <div class="wrap">
      <div class="grid g3 tilt-wrap">
        ${product({art:G.svgCctv('bullet'),h:'CCTV Systems',p:'HD video surveillance for monitoring, deterrence and evidence, with night vision and remote mobile viewing.',chips:['HD','Night vision','Mobile viewing'],spec:['Indoor and outdoor cameras','Remote monitoring and playback','Motion detection and recording'],app:'Perimeters, entrances, corridors, yards',order:'CCTV System',over:'Remote mobile viewing'})}
        ${product({art:G.svgCctv('dome'),h:'Dome and Internal Cameras',p:'Discreet internal coverage for receptions, corridors and retail floors where a bullet camera would dominate the room.',chips:['HD','Indoor'],spec:['Vandal resistant housings','Wide internal coverage'],app:'Receptions, classrooms, shop floors',order:'Dome CCTV Cameras',over:'Discreet internal coverage'})}
        ${product({art:G.svgIntruder('pir'),h:'Intruder Alarms',p:'Infrared and motion triggered detection for entry points and internal spaces, integrating with CCTV.',chips:['PIR','Works with CCTV'],spec:['Motion detection','Door and window sensors','Alarm panels','Sirens and silent alerts'],app:'Offices, warehouses, schools, homes',order:'Intruder Alarm System',over:'Sirens and silent alerts'})}
        ${product({art:G.svgIntruder('siren'),h:'External Sirens and Strobes',p:'Audible and visual alerting at the boundary, so an intrusion is announced outside as well as inside.',spec:['Sounder and strobe','Tamper protection'],app:'Building perimeters',order:'External Siren',over:'Tamper protected'})}
        ${product({art:G.svgAccess('keypad'),h:'Card and PIN Access Control',p:'Manage who goes where, and keep a record of it. Single door through to networked multi door systems.',chips:['Card','PIN','Network ready'],spec:['Single door and multi door systems','Staff entry tracking','Networked configurations'],app:'Offices, server rooms, stores, labs',order:'Access Control (Card / PIN)',over:'Tracks staff entry'})}
        ${product({art:G.svgAccess('biometric'),h:'Biometric Access Control',p:'Fingerprint credentials for areas where a card that can be lent or lost is not good enough.',chips:['Biometric'],spec:['Fingerprint enrolment','Integrates with door hardware'],app:'Server rooms, cash offices, restricted areas',order:'Biometric Access Control',over:'Fingerprint enrolment'})}
      </div>
      <div class="btn-row reveal" style="margin-top:30px">
  ...
  <section class="section">
    <div class="wrap">
      <div class="split">
        ...
        <div class="reveal" data-d="1">
          <div class="figure figure--tall"><img src="${IMG.servicing}" alt="Fire extinguishers gathered for inspection and refilling" loading="lazy" decoding="async">
...
const CATALOGUE = [
  ...
  {cat:'protection', art:()=>G.svgExtinguisher('water'), h:'Water and Water Mist', p:'Cools burning solids, with reduced electrical risk in the mist variants.', chips:['KEBS certified'], app:'Stores, dormitories, paper and textile risks', over:'Class A risks'},
  {cat:'protection', img:IMG.rangeGroup, fill:true, h:'Mobile Trolley Units', p:'High capacity wheeled units for large open risks where a portable will not cover the area.', chips:['KEBS certified','Wheeled'], app:'Factories, yards, fuel bays', over:'For large open risk areas'},
  {cat:'protection', art:()=>G.svgBlanket(), h:'Fire Blankets', p:'Wall mounted and fire resistant, sized from 1m x 1m up to 1.8m x 1.8m.', chips:['Wall mounted'], app:'Kitchens, laboratories, restaurants, offices, homes', over:'Smothers at the source'},
  ...
  {cat:'detection', art:()=>G.svgSounder(), h:'Sounders and Backup Batteries', p:'Sirens, strobes and standby batteries for fire alarm systems.', chips:['BS 5839'], app:'All system types', over:'Keeps the system alive on mains failure'},

  {cat:'emergency', art:()=>G.svgEmergencyLight(), h:'Emergency Lighting', p:'LED emergency luminaires and exit lights that illuminate escape routes on mains failure.', chips:['OSHA / NEMA','LED'], app:'Stairwells, corridors, exits', over:'Tested and maintained'},
  {cat:'emergency', art:()=>G.svgSign('exit'), h:'Exit and Directional Signs', p:'Photoluminescent and LED signage for emergency routes and final exits.', chips:['Photoluminescent','NEMA / OSHA'], app:'All building types', over:'Readable without power'},
  {cat:'emergency', art:()=>G.svgSign('extinguisher'), h:'Fire Point Signs', p:'Marks extinguisher and fire point locations so they can be found by someone who has never looked before.', chips:['Photoluminescent'], app:'Corridors, workshops, kitchens', over:'Makes equipment findable'},
  {cat:'emergency', art:()=>G.svgSign('assembly'), h:'Assembly Point Signs', p:'Defines where people gather after evacuation, which is what turns a drill into a roll call.', app:'Car parks, fields, courtyards', over:'Supports the roll call'},

  {cat:'security', art:()=>G.svgCctv('bullet'), h:'CCTV Cameras', p:'HD indoor and outdoor cameras with night vision and remote mobile viewing.', chips:['HD','Night vision'], app:'Perimeters, entrances, yards', over:'Remote mobile viewing'},
  {cat:'security', art:()=>G.svgCctv('dome'), h:'Dome Cameras', p:'Discreet internal coverage for receptions, classrooms and shop floors.', chips:['HD','Indoor'], app:'Receptions, classrooms, shop floors', over:'Vandal resistant housing'},
  {cat:'security', art:()=>G.svgAccess('keypad'), h:'Access Control Devices', p:'Card and PIN readers for single door and networked multi door systems.', chips:['Card','PIN','Network ready'], app:'Offices, stores, server rooms', over:'Tracks staff entry'},
  {cat:'security', art:()=>G.svgAccess('biometric'), h:'Biometric Readers', p:'Fingerprint credentials for restricted areas where a card is not enough.', chips:['Biometric'], app:'Server rooms, cash offices', over:'Fingerprint enrolment'},
  {cat:'security', art:()=>G.svgIntruder('pir'), h:'Intruder Alarm Devices', p:'PIR motion sensors, door and window contacts and alarm panels.', chips:['PIR'], app:'Offices, warehouses, homes', over:'Works alongside CCTV'},
  {cat:'security', art:()=>G.svgIntruder('siren'), h:'External Sirens and Strobes', p:'Boundary alerting with sounder, strobe and tamper protection.', app:'Building perimeters', over:'Tamper protected'},
  ...
  {cat:'firstaid', art:()=>G.svgFirstAid(), h:'First Aid Kits', p:'Workplace first aid kits to accompany First Aid training and provision.', app:'Workplaces, schools, clinics', over:'Pairs with First Aid training'}
];
