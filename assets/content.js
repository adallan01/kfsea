...
const IMG = {
  ...
  emgLightTest:'assets/img/p-emg-light-test.webp',
...
const EXPLORER = [
  ...
  {
    key:'security', label:'Security Systems', ico:'camera', art:'cctv',
        ...
        <div class="exp-panel">
          ${EXPLORER.map((e,i)=>`
            <div class="pane${i===0?' on':''}" id="pane-${e.key}" role="tabpanel" aria-labelledby="tab-${e.key}" data-pane="${i}">
              <div class="pane-media${e.art?' is-art':''}">${
                e.art==='cctv' ? G.svgCctv('bullet') : `<img src="${e.img}" alt="${e.label}" loading="lazy" decoding="async">`
              }</div>
              <div class="pane-body">
                <h3>${e.h}</h3><p>${e.p}</p>
  ...
  <section class="section">
    <div class="wrap">
      ...
      <div class="grid g4 tilt-wrap">
        ...
        ${product({art:G.svgDetector('smoke'),h:'Smoke Detectors',p:'Early stage smoke detection with audible warning, wired and wireless.',chips:['Wired / wireless'],app:'Residential, commercial, institutional',order:'Smoke Detectors',over:'Integrates with alarm systems'})}
    ...
    <div class="wrap">
      ...
      <div class="trainer reveal" id="trainer">
        <div class="trainer-media">
          ${[
            {img:IMG.handling},{img:IMG.wallSign},{art:'firstaid'},{art:'assembly'},{img:IMG.hoseInterior}
  ...
  <section class="section">
    <div class="wrap">
      <div class="split">
        ...
        <div class="art-frame reveal" data-d="1">${G.svgHeroBuilding()}</div>
      </div>
    </div>
  </section>
