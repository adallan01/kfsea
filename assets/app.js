    ...
    <div class="wrap">
      <div class="navbar">
        ...
        <div class="nav-cta">
          ...
          <button class="theme-btn" id="theme-btn" type="button" aria-label="Switch to dark theme">
            <svg class="ic-sun" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><circle cx="12" cy="12" r="4.2"/><path d="M12 2v2.4M12 19.6V22M2 12h2.4M19.6 12H22M4.9 4.9l1.7 1.7M17.4 17.4l1.7 1.7M19.1 4.9l-1.7 1.7M6.6 17.4l-1.7 1.7"/></svg>
            <svg class="ic-moon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 14.5A8.5 8.5 0 0 1 9.5 4a8.5 8.5 0 1 0 10.5 10.5z"/></svg>
          </button>
...
function wireChrome(){
  ...
  /* ---------------- Theme ----------------
     The choice is remembered per visitor. With no stored choice the site
     follows the operating system, and keeps following it if that changes. */
  ...
  const applyTheme = mode => {
    ...
    if(meta) meta.setAttribute('content', mode === 'dark' ? '#101215' : '#202225');
    if(themeBtn) themeBtn.setAttribute('aria-label',
      mode === 'dark' ? 'Switch to light theme' : 'Switch to dark theme');
  };

  let stored = null;
  try { stored = localStorage.getItem('kfsea-theme'); } catch(e){}
  applyTheme(stored || (darkQuery.matches ? 'dark' : 'light'));

  const onSystemChange = e => {
    let saved = null;
    try { saved = localStorage.getItem('kfsea-theme'); } catch(err){}
  ...
  if(themeBtn) themeBtn.addEventListener('click', ()=>{
    ...
    applyTheme(next);
    try { localStorage.setItem('kfsea-theme', next); } catch(e){}
  });
